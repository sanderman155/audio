...
std::vector<short> load_wav(const std::string &wav_file_path, int *sample_rate,
                            WavHeader* header_to_save) {
    // open file
...
int save_wav(std::vector<short> audio, WavHeader header, int numSeg, int start, int end) {
    std::ofstream file;
    file.open((std::to_string(numSeg) + ".wav"), std::ios::binary);
    int size = (end - start) * header.sampleRate;
    WavHeader header_to_save = header;
    header_to_save.chunkSize = (2 * size + 44) - 8;
    header_to_save.subchunk2Size = 2 * size;
    std::vector<short> audio_to_save(size);
    for (int i = start * header.sampleRate, j = 0; i < end * header.sampleRate; i++, j++) {
        audio_to_save[j] = audio[i];
    }
    file.write((char *)&header_to_save, sizeof(header_to_save));
    file.write((char *)(audio_to_save.data()), audio_to_save.size() * 2);
    file.close();
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    int sample_rate = 0;
    WavHeader header_to_save;
    std::vector<short> audio = load_wav(wav_file_path, &sample_rate, &header_to_save);
...
/*    std::cout << segments_energy.size() << std::endl;
    std::cout << vad_mask.size() << std::endl;
    std::cout << segments.size() << std::endl;
    for (auto it: segments_energy) {
        std::cout << it << std::endl;
    }
    for (auto it: vad_mask) {
        std::cout << it << std::endl;
    }
    for (auto it: segments_energy) {
        std::cout << it << std::endl;
    ...
    for (int i = 0; i < segments.size(); i++) {
        save_wav(audio, header_to_save, i, segments[i].start, segments[i].stop);
    }
