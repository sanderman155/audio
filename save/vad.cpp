...
std::vector<short> load_wav(const std::string &wav_file_path, int *sample_rate, WavHeader *header) {
  // open file
  ...
  input_stream.read((char *)&new_header, sizeof(new_header));
  if (strncmp(new_header.format, "WAVE", 4) != 0) {
    print(wav_file_path + ": file is not wav.");
    exit(2);
  }
  print(wav_file_path);
  print_wav_header_info(new_header);
  *sample_rate = new_header.sampleRate;
  *header = new_header;
...
int save(std::vector<short> audio, std::vector<Segment> segments, WavHeader header, int i, int sd) {
    ...
    file.write((char *)&header_new, sizeof(header_new));
    file.write((char *)(audio_segment.data()), audio_segment.size() * 2);
    file.close();
    return 0;
}
